...
__global__ void compute_histogram_shared_dico(const ELEMENT_TYPE *array,
  ...
  // Création d'un histogramme temporaire pour ce thread
  int tmp_histogram[20] = {0}; // Assurez-vous que 32 >= nb_bins
  ...
  // Reduction de l'histogramme temporaire dans l'histogramme global
  for (int i = 0; i < nb_bins; i++) {
    atomicAdd(&global_histogram[i], tmp_histogram[i]);
  }
}
