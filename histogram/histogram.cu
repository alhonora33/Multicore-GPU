...
static void init_settings(struct s_settings **pp_settings) {
  ...
  struct s_settings *p_settings =
      (struct s_settings *)calloc(1, sizeof(*p_settings));
  if (p_settings == NULL) {
...
static void parse_cmd_line(int argc, char *argv[],
  ...
  while (i < argc) {
    if (strcmp(argv[i], "--array-len") == 0) {
      ...
      int fp_class = fpclassify(value);
      if ((fp_class != FP_NORMAL) && (fp_class != FP_ZERO)) {
...
static void allocate_array(ELEMENT_TYPE **p_array,
                           struct s_settings *p_settings) {
  assert(*p_array == NULL);
...
static void init_array_random(ELEMENT_TYPE *array,
                              struct s_settings *p_settings) {
  ...
  for (i = 0; i < p_settings->array_len; i++) {
    ELEMENT_TYPE value =
        scale * ((ELEMENT_TYPE)rand()) / (1.0 + (ELEMENT_TYPE)(RAND_MAX)) +
        offset;
    array[i] = value;
  }
}

static void print_array(const ELEMENT_TYPE *array,
                        struct s_settings *p_settings) {
  printf("[");
  int j = 0;
  int i;
...
static void write_array_to_file(FILE *file, const ELEMENT_TYPE *array,
                                struct s_settings *p_settings) {
  int i;
  int ret;
...
static void allocate_histogram(int **p_histogram,
                               struct s_settings *p_settings) {
...
static void print_histogram(const int *histogram,
                            struct s_settings *p_settings) {
  const ELEMENT_TYPE offset = p_settings->lower_bound;
  const ELEMENT_TYPE scale = p_settings->upper_bound - p_settings->lower_bound;
...
static void write_histogram_to_file(FILE *file, const int *histogram,
                                    struct s_settings *p_settings) {
  int i;
  int ret;
...
static void print_settings_csv(struct s_settings *p_settings) {
  printf("%d,%d,%d", p_settings->array_len, p_settings->nb_bins,
         p_settings->nb_repeat);
}
...
static void print_results_csv(int rep, double timing_in_seconds,
                              int check_status) {
...
static void naive_compute_histogram(const ELEMENT_TYPE *array, int *histogram,
                                    struct s_settings *p_settings) {
  memset(histogram, 0, p_settings->nb_bins * sizeof(*histogram)); // zero out
  ...
  {
    const ELEMENT_TYPE offset = p_settings->lower_bound;
    const ELEMENT_TYPE scale =
        p_settings->upper_bound - p_settings->lower_bound;
...
static void omp_compute_histogram(const ELEMENT_TYPE *array, int *histogram,
                                  struct s_settings *p_settings) {
...
#pragma omp parallel for schedule(dynamic, 32)                                 \
    reduction(+ : histogram[ : p_settings->nb_bins])
...
static void omp_compute_histogram_dico(const ELEMENT_TYPE *array,
                                       int *histogram,
                                       struct s_settings *p_settings) {
...
#pragma omp parallel for schedule(dynamic, 32)                                 \
  ...
  for (int i = 0; i < p_settings->array_len; i++) {
    ...
    int low = 0, high = p_settings->nb_bins - 1;
    while (low <= high) {
...
__global__ void compute_histogram_shared_dico(const ELEMENT_TYPE *array,
  ...
  if (idx < array_len) {
    ...
    while (low <= high) {
      int mid = low + (high - low) / 2;
      if (value >= bounds[mid] && value < bounds[mid + 1]) {
        atomicAdd(&local_histogram[mid], 1);
        break;
      } else if (value < bounds[mid]) {
        high = mid - 1;
      } else {
...
static void cuda_compute_histogram(const ELEMENT_TYPE *array, int *histogram,
                                   struct s_settings *p_settings) {

  ELEMENT_TYPE *bounds;
  ELEMENT_TYPE *d_array;
  int *d_histogram;

  cudaMallocManaged(&d_array, p_settings->array_len * sizeof(ELEMENT_TYPE));
  cudaMallocManaged(&d_histogram, p_settings->nb_bins * sizeof(int));
  cudaMallocManaged(&bounds, (p_settings->nb_bins + 1) * sizeof(ELEMENT_TYPE));
  ...
  for (int i = 0; i < p_settings->nb_bins; i++) {
    d_histogram[i] = 0;
  }
...
static void run(const ELEMENT_TYPE *array, int *run_histogram,
  ...
  omp_compute_histogram_dico(array, run_histogram, p_settings);
...
static int check(const ELEMENT_TYPE *array, int *check_histogram,
                 const int *run_histogram, struct s_settings *p_settings) {
